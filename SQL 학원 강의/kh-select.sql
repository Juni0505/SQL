...
SELECT emp_name, email, phone, hire_date FROM EMPLOYEE e ;

-- 5. EMPLOYEE테이블의 고용일, 사원 이름, 월급 조회
SELECT hire_date, emp_name, salary FROM EMPLOYEE e ;

-- 6. EMPLOYEE테이블에서 이름, 연봉, 총수령액(보너스포함), 실수령액(총수령액 - (연봉*세금 3%)) 조회
SELECT emp_name, salary*12 연봉, 
COALESCE(SALARY*12+(SALARY*12*BONUS), SALARY*12) 총수령액, 
COALESCE((SALARY*12+(SALARY*12*BONUS))-(SALARY*12*0.03), SALARY*12-SALARY*12*0.03) 실수령액
FROM EMPLOYEE e ;

-- 7. EMPLOYEE테이블에서 SAL_LEVEL이 S1인 사원의 이름, 월급 고용일, 연락처 조회
SELECT * FROM EMPLOYEE WHERE sal_level = 'S1';

-- 8. EMPLOYEE테이블에서 실수령액(6번 참고)이 5천만원 이상인 사원의 이름, 월급, 실수령액, 고용일 조회
SELECT emp_name, salary, COALESCE((SALARY*12+(SALARY*12*BONUS))-(SALARY*12*0.03), SALARY*12-SALARY*12*0.03) 실수령액,
hire_date FROM EMPLOYEE 
WHERE COALESCE((SALARY*12+(SALARY*12*BONUS))-(SALARY*12*0.03), SALARY*12-SALARY*12*0.03) > 50000000;
...
BETWEEN TO_DATE(19900101,'yyyymmdd') AND TO_DATE(20010101,'yyyymmdd');

-- 12. EMPLOYEE테이블에서 이름 끝이 '연'으로 끝나는 사원읭 이름 조회
SELECT emp_name FROM EMPLOYEE WHERE emp_name LIKE '%연';

-- 13. EMPLOYEE테이블에서 전화번호 처음 3자리가 010이 아닌 사원의 이름, 전화번호를 조회
SELECT emp_name, PHONE FROM EMPLOYEE WHERE PHONE NOT LIKE '__0%';
...
SELECT emp_name, REPLACE(EMP_NO, 8, 14, '*')  FROM EMPLOYEE; 
...
-- 주민번호 앞자리와 뒷자리의 합 조회
SELECT emp_name, SUBSTR(EMP_NO, 1, 6) 앞자리 ,substr(emp_no, 8, 12) 뒷자리, SUBSTR(EMP_NO, 1, 6)+substr(emp_no, 8, 12) 합
FROM EMPLOYEE WHERE EMP_ID = '201';

-- 24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT sum(SALARY+(SALARY*NVL(bonus, 0))* 12) 연봉 FROM EMPLOYEE WHERE DEPT_CODE = 'D5';
